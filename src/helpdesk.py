...
CATEGORY_KEYWORDS = {
    "Network": [
        "wifi", "wi-fi", "internet", "network", "vpn",
        "ethernet", "connection", "connectivity"
    ],
    "Hardware": [
        "laptop", "desktop", "keyboard", "mouse", "monitor",
        "screen", "printer", "battery", "charger", "hardware"
    ],
    "Software": [
        "software", "application", "app", "crash", "install",
        "update", "excel", "word", "outlook", "browser"
    ],
    "Account & Access": [
        "password", "login", "log in", "account", "access",
        "permission", "locked", "authentication", "mfa"
    ],
    "Email": [
        "email", "mailbox", "gmail", "mail", "attachment", "outlook"
    ],
    "Security": [
        "phishing", "malware", "virus", "suspicious",
...
class HelpdeskAgent:
    ...
    def _build_retriever(self):
        texts = [
            self.clean(
                f"{r['title']} {r['category']} {r['solution']}"
            )
    ...
    def predict_priority(self, text):
        ...
        # Security incidents receive higher priority.
        ...
        if not factors:
            factors.append(
                "no strong urgency or broad-impact signals detected"
            )

        return priority, factors

    def retrieve_faq(self, text, category):
        q = self.retriever.transform(
            [self.clean(text)]
        )
    ...
    def analyze(self, text):
        ...
        priority, factors = (
            self.predict_priority(text)
        )

        faq, faq_score = (
            self.retrieve_faq(text, category)
        )

        reason = (
            f"The model classified the request as "
            f"'{category}' with "
            f"{confidence * 100:.1f}% confidence. "
            f"Priority was set to '{priority}' "
            f"based on the detected signals: "
            f"{', '.join(factors)}."
        )
    ...
    def _draft_response(
        ...
        urgency = (
            "Because this appears to be a high-impact "
            "or security-related issue, please avoid "
            "making further changes to the affected "
            "system and contact the IT/security team "
            "immediately."
            if priority in ["Critical", "High"]
            else
            "We have reviewed your request and "
            "identified a relevant troubleshooting article."
        )
        ...
        return (
            ...
            f"Priority: {priority}\n\n"
            f"{urgency}\n\n"
            f"Recommended guidance — "
            f"{faq['title']}:\n"
            f"{faq['solution']}\n\n"
            "If the issue continues, please reply with "
            "a screenshot or the exact error message "
            "and the approximate time the problem started. "
            "An IT support engineer can then investigate "
            "further.\n\n"
            "Regards,\n"
            "IT Support"
        )